...
hit_score   当前轮命中的节点的得分
user_query  用户当轮输入
missing_slot  当前欠缺的槽位
policy 当前策略，策略2种 ask和answer
"""
...
class DialogueSystem:
    def __init__(self, all_nodes_info=None, slot_info=None):
        self.all_nodes_info = all_nodes_info if all_nodes_info is not None else {}
        self.slot_info = slot_info if slot_info is not None else {}
        self.load()  # 完成所有的资源加载


    def load(self):       
        #加载场景脚本
        # self.all_nodes_info={} #id为key，value为节点信息
        self.load_scenario("scenario-买衣服.json")      
        #加载填槽脚本
        # self.slot_info={} #id为槽位名称，value是[query,values]
    ...
    def sentence_similarity(self,string1,string2):
        #使用jaccard距离
        score=len(set(string1).intersection(set(string2)))/len(set(string1).union(set(string2)))
        return score
...
if __name__=='__main__':
    ...
    # print(dialogueSystem.all_nodes_info)
    # print(dialogueSystem.slot_info)
    # memory=dialogueSystem.run("我想买红色短袖衣服",{})
    # print(memory)
    memory={}
    while True:
        user_query=input('请输入您的问题：')
        memory=dialogueSystem.run(user_query,memory)
